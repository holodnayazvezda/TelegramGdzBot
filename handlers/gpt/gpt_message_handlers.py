...
from handlers.gpt.gpt_functions import (change_gpt_version, generate_and_send_answer, clear_chat_gpt_conversation,
                                        add_user_to_queue_and_start_generating)
...
async def chat_gpt_starter(message, bot_instance: BotInfo) -> None:
    ...
    rest_of_amount_of_image_sending, amount_of_unsucessful_requests_to_ocr_api = (
        ...
        (await get_amount_of_requests_for_user("./data/databases/quantity_of_requests.sqlite3",
                                               "quantity_of_requests_to_ocr_space", message.from_user.id)),
        await get_amount_of_requests_for_user("./data/databases/quantity_of_requests.sqlite3",
    ...
    if ('id_of_message_with_markup' not in dictionary_used_in_this_function or not
        ...
        chatgpt_main_markup.add(types.KeyboardButton(
            text=f"🔁 Переключиться на {'gpt-4' if (model == 'gpt-3.5-turbo' or not model) else 'gpt-3.5-turbo'}"))
        chatgpt_main_markup.add(types.KeyboardButton(text='↩ Назад в главное меню'))
        await send_message(user_id=message.from_user.id, bot=bot_instance.bot, bot_id=bot_instance.bot_id,
        ...
        message_id = await try_edit_or_send_message(user_id=message.from_user.id, bot=bot_instance.bot,
