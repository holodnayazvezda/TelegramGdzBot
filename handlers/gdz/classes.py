...
async def gdz_starter(message, bot_instance: BotInfo) -> None:
    ...
    if users_dict:
        if isinstance(message, types.Message):
            Thread(target=async_functions_process_starter, args=(active_now, [
                str(message.from_user.id), message.chat.id, bot_instance.bot_id])).start()
            ...
            try:
                ...
                message_id = await try_edit_or_send_message(user_id=call.from_user.id, bot=bot_instance.bot,
                                                            bot_id=bot_instance.bot_id, chat_id=call.message.chat.id,
                                                            text='Выбери свой класс!', reply_markup=markup)
                users_dict['id_of_message_with_markup'] = message_id
